...
class TilesLayout extends Component {
  constructor(props) {
    ...
    const hexagons = GridGenerator.parallelogram(-1, 1, -1, 2).map((hexagon, index) => {
      return Object.assign({}, hexagon, {
        text: ``,
        image: `https://media.defense.gov/2006/Jul/05/2000790798/-1/-1/0/060705-F-ZZ000-453.JPG`
      });
    })
    this.state = { hexagons };
  }
  ...
  render() {
    const { hexagons } = this.state;
    return (
      <Layout className="tiles" size={{ x: 3, y: 3 }} flat={false} spacing={1.01} origin={{ x: 40, y: -20 }}>
        {
          hexagons.map((hex, i) => (
            <Hexagon
              key={i}
